...
class BertForEntity(BertPreTrainedModel):
    def __init__(self, config, num_ner_labels, head_hidden_dim=150, width_embedding_dim=150, max_span_length=8):
        ...
        self.is_overlapped_embedding=nn.Embedding(max_span_length + 1, width_embedding_dim)

        self.ner_classifier = nn.Sequential(
            FeedForward(input_dim=config.hidden_size * 2 + width_embedding_dim+width_embedding_dim,
                        num_layers=2,
    ...
    def _get_span_embeddings(self, input_ids, spans, spans_is_overlapped=None,token_type_ids=None,attention_mask=None):
        ...
        spans_is_overlapped_embedding=self.is_overlapped_embedding(spans_is_overlapped)
        ...
        spans_embedding = torch.cat((spans_start_embedding, spans_end_embedding, spans_width_embedding,spans_is_overlapped_embedding), dim=-1)
        """
        spans_embedding: (batch_size, num_spans, hidden_size*2+embedding_dim)
        """
        return spans_embedding

    def forward(self, input_ids, spans, spans_mask, spans_is_overlapped,spans_ner_label=None,token_type_ids=None, attention_mask=None):
        spans_embedding = self._get_span_embeddings(input_ids, spans, token_type_ids=token_type_ids,spans_is_overlapped=spans_is_overlapped,
                                                    attention_mask=attention_mask)
        ffnn_hidden = []
        overlap_ffnn_hidden = []
        ...
        hidden = spans_embedding
        for layer in self.overlap_classifier:
            hidden = layer(hidden)
            overlap_ffnn_hidden.append(hidden)
        overlap_logits = overlap_ffnn_hidden[-1]
        ...
        if spans_ner_label is not None:
            ...
            if attention_mask is not None:
                ...
                active_overlap_logits=overlap_logits.view(-1, overlap_logits.shape[-1])
                active_overlap_labels = torch.where(
                    active_loss, spans_is_overlapped.view(-1), torch.tensor(loss_fct.ignore_index).type_as(spans_is_overlapped)
                )
                overlap_loss=loss_fct(active_overlap_logits,active_overlap_labels)

                loss =ner_loss+overlap_loss
            else:
                loss = loss_fct(logits.view(-1, logits.shape[-1]), spans_ner_label.view(-1))
            return loss, logits, overlap_logits,spans_embedding
        else:
            return logits,overlap_logits, spans_embedding, spans_embedding
...
class AlbertForEntity(AlbertPreTrainedModel):
    def __init__(self, config, num_ner_labels, head_hidden_dim=150, width_embedding_dim=150, max_span_length=8):
        ...
        self.hidden_dropout = nn.Dropout(config.hidden_dropout_prob)
        self.width_embedding = nn.Embedding(max_span_length + 1, width_embedding_dim)
        # self.over_lapped = nn.Embedding(1,over_lapped_with)
    ...
    def forward(self, input_ids, spans, spans_mask, spans_ner_label=None, spans_is_overlapped=None,token_type_ids=None, attention_mask=None):
...
class EntityModel():
    ...
    def _get_input_tensors(self, tokens, spans, spans_ner_label,spans_is_overlapped):
        start2idx = []
        end2idx = []
        ...
        spans_is_overlapped_tensor=torch.tensor([spans_is_overlapped])

        return tokens_tensor, bert_spans_tensor, spans_ner_label_tensor,spans_is_overlapped_tensor
    ...
    def _get_input_tensors_batch(self, samples_list, training=True):
        ...
        sentence_length = []
        spans_is_overlapped_tensor_list=[]
        max_tokens = 0
        max_spans = 0
        for sample in samples_list:
            ...
            spans_is_overlapped=sample['spans_is_overlapped']
            tokens_tensor, bert_spans_tensor, spans_ner_label_tensor,spans_is_overlapped_tensor = self._get_input_tensors(tokens, spans,
                                                                                               spans_ner_label,spans_is_overlapped)
            ...
            spans_is_overlapped_tensor_list.append(spans_is_overlapped_tensor)
        ...
        final_spans_is_overlapped_tensor = None
        for tokens_tensor, bert_spans_tensor, spans_ner_label_tensor,spans_is_overlapped_tensor in zip(tokens_tensor_list, bert_spans_tensor_list,
                                                                            spans_ner_label_tensor_list,spans_is_overlapped_tensor_list):
            # padding for tokens
            ...
            if spans_pad_length > 0:
                ...
                spans_is_overlapped_tensor=torch.cat((spans_is_overlapped_tensor, mask_pad), dim=1)
            ...
            if final_tokens_tensor is None:
                ...
                final_spans_is_overlapped_tensor=spans_is_overlapped_tensor
                ...
                final_spans_is_overlapped_tensor=torch.cat((final_spans_is_overlapped_tensor, spans_is_overlapped_tensor), dim=0)
        ...
        return final_tokens_tensor, final_attention_mask, final_bert_spans_tensor, final_spans_mask_tensor, final_spans_ner_label_tensor, final_spans_is_overlapped_tensor,sentence_length

    def run_batch(self, samples_list, try_cuda=True, training=True):
        # convert samples to input tensors
        tokens_tensor, attention_mask_tensor, bert_spans_tensor, spans_mask_tensor, spans_ner_label_tensor, spans_is_overlapped_tensor,sentence_length = self._get_input_tensors_batch(
        ...
        if training:
            ...
            ner_loss, ner_logits, overlap_logits,spans_embedding = self.bert_model(
                input_ids=tokens_tensor.to(self._model_device),
                spans=bert_spans_tensor.to(self._model_device),
                spans_mask=spans_mask_tensor.to(self._model_device),
                spans_is_overlapped=spans_is_overlapped_tensor.to(self._model_device),
            ...
            with torch.no_grad():
                ner_logits,overlap_logits, spans_embedding, last_hidden = self.bert_model(
                    input_ids=tokens_tensor.to(self._model_device),
                    spans=bert_spans_tensor.to(self._model_device),
                    spans_mask=spans_mask_tensor.to(self._model_device),
                    spans_is_overlapped=spans_is_overlapped_tensor.to(self._model_device),
            ...
            _,predicted_overlap_label=overlap_logits.max(2)
            predicted_label = predicted_label.cpu().numpy()
            predicted_overlap_label =predicted_overlap_label.cpu().numpy()
            ...
            for i, sample in enumerate(samples_list):
                ...
                for j in range(len(sample['spans'])):
                    ...
                    overlap.append(predicted_overlap_label[i][j])
