...
from shared.over_lapped_data_structures import Dataset
from shared.const import task_ner_labels, get_labelmap
from entity.over_lapped_utils import convert_dataset_to_samples, batchify, NpEncoder
from entity.over_lapped_models import EntityModel
...
def output_ner_predictions(model, batches, dataset, output_file):
    ...
    for i in range(len(batches)):
        ...
        pred_is_overlap=output_dict['pred_overlap']
        for sample, preds,over_lap in zip(batches[i], pred_ner,pred_is_overlap):
            off = sample['sent_start_in_doc'] - sample['sent_start']
            k = sample['doc_key'] + '-' + str(sample['sentence_ix'])
            ner_result[k] = []
            for span, pred,lap in zip(sample['spans'], preds,over_lap):
                span_id = '%s::%d::(%d,%d)'%(sample['doc_key'], sample['sentence_ix'], span[0]+off, span[1]+off)
                if pred == 0:
                    continue
                ner_result[k].append([span[0]+off, span[1]+off, ner_id2label[pred],lap])
...
def evaluate(model, batches, tot_gold):
    ...
    for i in range(len(batches)):
        output_dict = model.run_batch(batches[i], training=False)
        pred_ner = output_dict['pred_ner']
        pred_overlap = output_dict['pred_overlap']
        for sample, preds in zip(batches[i], pred_ner):
            for gold, pred in zip(sample['spans_label'], preds):
                ...
                if pred != 0:
                    tot_pred += 1
        for sample, preds in zip(batches[i], pred_overlap):
            for gold, pred in zip(sample['spans_is_overlapped'], preds):
                o_tot += 1
                if pred == gold:
                    o_cor += 1
                if pred != 0 and gold != 0 and pred == gold:
    ...
    logger.info('overlap_err:%.5f' % (o_err/o_p))
    return f1
...
if __name__ == '__main__':
    ...
    parser.add_argument('--task', type=str, default=None, required=True, choices=['ace04', 'ace05', 'scierc','our_data'])
